...
| [07-ASan与UBSan内存安全验证.md](07-ASan与UBSan内存安全验证.md) | 内存安全这条腿的专档：为什么只有 clang64 能跑、脚本跑什么、`ldb_arena_allocate_aligned` 与 logger `destroy` 钩子等每一处改动、负向对照的做法与坑、sanitizer 覆盖不到的部分 |
...
- 想复跑或扩展内存安全验证 → 07（ASan/UBSan 这条腿的用法、逐文件改动、
  负向对照的坑，以及它测不到的部分）。
