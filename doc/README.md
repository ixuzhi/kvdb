...
| [10-兼容实现评估报告.md](10-兼容实现评估报告.md) | 2026-09-25 独立复核轮的**综合评价**：兼容性达成度（官方 c_test 直连、黄金比对三向一致、双后端 146/0）、优点与证据链、现存缺陷分级清单（1 critical / 6 major / 1 minor + 13 条测试装置隐患 + 24 组覆盖缺口）、**不能宣称什么的诚实边界**（弱内存序、snappy 字节等价、泄漏仅 Linux、跨后端无 sanitizer、故障注入未覆盖）、评分表与按风险/成本排序的行动项 |
| [11-检查检测评价方法论.md](11-检查检测评价方法论.md) | 统一评价手册：**五层检查**（L0 内建回归+官方 c_test 直连 / L1 跨引擎黄金比对 / L2 跨后端落盘比对 / L3 ASan+UBSan+LSan / L4 静态审计与逐函数对照）+ **三条横切原则**（负向对照、装置自身可信、结论带环境坐标）；各层跑什么与不证明什么、rubric 评分表、把"未覆盖"变"已覆盖"的补法路线；本轮新增三族手段（log initial-offset 参数化边界矩阵、并发 oracle 移植、大值/堆路径） |
| [12-环境支持矩阵.md](12-环境支持矩阵.md) | **当前支持状态的唯一权威页**：六套环境（w64devkit / MSYS2 msys·MINGW64·CLANG64 / 独立 Cygwin / 原生 Linux）的获取方式、三元组、Env 后端、sanitizer 可用性、专用对象树与能承担的腿；**§2 安装组合手册（命令级）**：宿主工具要求（w64devkit 无 bash、README 的"自动下载"与实际不符等诚实备注）、组合 A 最小可用 / B Windows 全功能 / C Cygwin / D WSL / E 原生 Linux / 离线与特殊路径、按目标选组合速查与安装后自检清单；§3 腿→环境判定规则与最短 PASS 路径；一键入口（`make doctor` / `make check` / `make matrix`）；2026-09-25 本机实测快照与全链验收记录；共存纪律六条与故障速查表。历史轮次细节见 doc/07/08/09 |
