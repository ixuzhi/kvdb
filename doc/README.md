...
| [08-Linux原生环境验证.md](08-Linux原生环境验证.md) | 首次原生 Linux（Debian 12 / glibc / gcc 12）全腿验证：环境矩阵、**为验证新装了什么软件与怎么卸干净**、四条腿逐条结果与留档路径、P0-10 的来龙去脉、两个 harness 陷阱（`make CFLAGS=` 吞掉 `+=`、静默链接陈旧归档）、snappy 压缩等价性实测结论、待 Windows 侧回归清单与延后 TODO |
...
- 想在别的平台/环境上接手验证，或想知道"这轮装了什么、怎么卸" →
  08（原生 Linux 全腿记录：环境矩阵、依赖与卸载、两个 harness 陷阱、
  延后 TODO 与待 Windows 回归清单）。
