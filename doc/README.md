...
| [14-第三体系-契约面审计.md](14-第三体系-契约面审计.md) | 2026-09-26 建立的第三套验证体系（入口 `bash tools/audit/audit_all.sh`）：以**官方 API 契约面**为场景来源的六腿——N1 接口面机械审计（签名/符号/枚举/版本 + 常设 ABI 替换）/ N2 选项×行为全矩阵 / N3 同步锁步差分 / N4 格式角落矩阵 / N5 交替生命周期+跨引擎修复 / N6 随机崩溃+跨引擎恢复；**N7 以已归档缺陷为标本的检出力矩阵**（M-1/F-2/M-5 被实证捕获）；新发现 D-1（env_posix 缺进程内持锁集合，doc/06 2-14）与 D-2（flush 粒度观察）、装置缺陷 A-1..A-8 |
| [13-独立验证报告.md](13-独立验证报告.md) | **正交验证体系**（入口 `make verify` / `bash tools/verify/verify_all.sh`，自验证 `verify_mutants.sh`）：§1 为什么要正交（既有证据的方向性盲区）、§2 架构设计（四原则 + 五腿结构 + 与 doc/11 五层的关系矩阵）、§3 从零可复现的验证步骤、§4 过程中 13 个探针/装置侧问题的定位与解决（全部由官方实现仲裁，含"差点误报成引擎缺陷"的 C API options 契约核对）、§5 **自验证**：4 变异体 × 5 方法的检测矩阵（揭示原套件放过快照可见性变异、等价变异陷阱）、§6 **对比验证**：同一 harness 同种子双跑官方/kvdb 全绿、§7 从零复跑结果、§8 **没有验证到的 10 条 TODO**（含边界声明） |
